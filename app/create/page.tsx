...
import {
  ...
  type Tone,
  type TributeData,
} from "@/lib/tribute-store";
import { buildTributeFromTemplate, type Tribute } from "@/lib/tribute-template";
    ...
    <div className="min-h-dvh parchment-bg pt-28 pb-24">
      <div className="mx-auto max-w-4xl px-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <p className="eyebrow">
            Step {step + 1} of {total} · Autosaved
          </p>
        </div>
        ...
        <div className="mt-8 flex flex-wrap justify-between gap-3">
          ...
          {!isLast ? (
            ...
            <GenerateButton
              onGenerate={async () => {
                const result = buildTributeFromTemplate(data);
                setTribute(result);
...
function GenerateButton({ onGenerate }: { onGenerate: () => Promise<void> }) {
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      >
        {loading ? (
          <>
            <span className="h-2 w-2 rounded-full bg-ivory animate-pulse" />
    ...
    <>
      ...
      <div className="grid gap-6">
        ...
        <Field
          ...
          hint="Moves, marriages, adventures, hard-won moments."
        >
          <TextArea
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center gap-6">
        <div className="w-56 h-56 rounded-full overflow-hidden bg-parchment border border-line flex items-center justify-center paper-grain">
          {data.photoDataUrl ? (
...
function StepReview({ data, set }: StepProps) {
  return (
    <>
      ...
      <div className="soft-card bg-ivory/70 p-6 md:p-8 space-y-3 text-sm">
        ...
        <p className="text-ink-soft">
          ...
          {data.residence || data.birthplace || "—"}
        </p>
        {data.personality && (
          <p className="text-ink-soft">
...
function TributeOutput({
  ...
  return (
    <div className="min-h-dvh parchment-bg pt-28 pb-24">
      <div className="mx-auto max-w-4xl px-6">
        {/* Header block: name, photo, quote — hidden when printing */}
        <motion.div
        ...
        >
          {photoDataUrl && (
            <div className="mb-6 w-32 h-32 rounded-full overflow-hidden border border-line paper-grain">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={photoDataUrl}
                alt={personName}
          ...
          <h1 className="mt-3 text-display text-4xl md:text-6xl">
            {personName}
          </h1>
          <p className="mt-6 text-editorial text-2xl text-forest max-w-2xl">
        ...
        {/* On-screen editable-looking view — hidden when printing */}
        <motion.div
          key={tab}
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}
          className="mt-6 soft-card p-8 md:p-12 paper-grain print:hidden"
        >
          <textarea
            value={text}
            onChange={() => {
              // Read-only for now. If you want real inline editing, wire this
              // to local state (or back into the store) instead of no-op.
            }}
        ...
        {/* Print-only: just the written tribute text, nothing else */}
        <div className="hidden print:block whitespace-pre-wrap text-lg leading-relaxed text-ink">
          {text}
        </div>
        ...
        {tribute.timeline?.length > 0 && (
          <div className="mt-16 print:hidden">
            ...
            <ol className="mt-6 space-y-4 border-l border-line pl-6">
              {tribute.timeline.map(
                (t: { year: string; event: string }, i: number) => (
